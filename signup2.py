...
def file_upload_and_display():
    ...
    if st.session_state.logged_in:
        ...
        # Show file history in the sidebar
        ...
        if user_files:
            for i, file in enumerate(user_files):
                file_name = file[0]
                unique_key = f"{i}_{file_name}"  # Create a unique key
...
def display_file_content(user_id, file_name):
    ...
    if file_contents:
        file_contents = file_contents[0].decode("utf-8")
        st.subheader(f"File Content of {file_name}:")
        st.code(file_contents, language="text")
    else:
